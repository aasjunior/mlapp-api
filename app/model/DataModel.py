...
class DataModel:
    ...
    def normalize_data(self):
        try:
            ...
            # Aplicar MinMaxScaler para colunas numéricas
            scaler = MinMaxScaler()
            data[self.numeric_cols] = scaler.fit_transform(data[self.numeric_cols])

            # Aplicar one-hot encoding para colunas categóricas
            encoder = OneHotEncoder(drop='first')
            encoded_cols = encoder.fit_transform(data[self.categorical_cols]).toarray()
            encoded_cols_df = pd.DataFrame(encoded_cols, columns=encoder.get_feature_names(self.categorical_cols))
            
            # Concatenar os dataframes
            data = pd.concat([data[self.numeric_cols], encoded_cols_df], axis=1)
